...
import com.appcore.core.task.MybatisDBTask;
...
public abstract class MybatisBaseServiceImpl
  implements MybatisBaseService
{
  ...
  public <T> Page<T> page(String pageSelectId, String pageCountSelectId, Map<String, Object> map, int pageIndex, int pageSize)
  {
    ...
    try
    {
      ...
      LOGGER.error("分页查询【数据集】异常，请检查Mapper.xml文件中是否有对应 select id 【{}】，检查resultMap是否合法!", new Object[] { pageSelectId });
      e.printStackTrace();
    }
  ...
  {
    String threadName = "DB_" + super.getClass().getSimpleName().replace("ServiceImpl", "").toUpperCase() + "_THREAD";
    return threadName;
  }
}